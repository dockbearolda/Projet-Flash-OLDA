...
// Mot de passe unique de l'atelier. Volontairement figé dans le code : c'est le
// seul mot de passe valide, identique en local et en prod, sans config.
const APP_PASSWORD = 'olda28280';

function sessionSecret(): string {
  const v = process.env.SESSION_SECRET;
  if (v && v.length >= 16) return v;
  // Secret stable par défaut : garde les sessions valides au travers des
  // redéploiements (Railway). Surcharger via SESSION_SECRET si besoin.
  return 'devis-flash-olda-session-secret-2026';
}
...
export async function login(password: string): Promise<string | null> {
  if (!safeEq(password, APP_PASSWORD)) return null;
